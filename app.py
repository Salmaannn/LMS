...
def add_book():
    ...
    if request.method == "POST" and form.validate():
        ...
        # Execute SQL Query
        cur.execute(
            "INSERT INTO books (id,title,author,average_rating,isbn,isbn13,language_code,num_pages,ratings_count,text_reviews_count,publication_date,publisher,total_quantity,available_quantity) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
            [
                form.id.data,
                form.title.data,
                form.author.data,
...
def import_books():
    ...
    if request.method == "POST" and form.validate():
        ...
        msg = (
            str(no_of_books_imported)
            + "/"
            + str(form.no_of_books.data)
            + " books have been imported. "
        )
        msgType = "success"
        if no_of_books_imported != form.no_of_books.data:
            msgType = "warning"
            if len(repeated_book_ids) > 0:
                msg += (
                    str(len(repeated_book_ids))
                    + " books were found with already exisiting IDs."
                )
            else:
                msg += (
                    str(form.no_of_books.data - no_of_books_imported)
...
def edit_book(id):
    ...
    if request.method == "POST" and form.validate():
        ...
        if form.id.data != id:
            ...
            if book:
                ...
                return render_template(
                    "edit_book.html", form=form, error=error, book=form.data
                )
        ...
        # Execute SQL Query
        cur.execute(
            "UPDATE books SET id=%s,title=%s,author=%s,average_rating=%s,isbn=%s,isbn13=%s,language_code=%s,num_pages=%s,ratings_count=%s,text_reviews_count=%s,publication_date=%s,publisher=%s,total_quantity=%s,available_quantity=%s WHERE id=%s",
            [
                form.id.data,
                form.title.data,
                form.author.data,
...
def issue_book():
    ...
    if request.method == "POST" and form.validate():
        ...
        cur.execute(
            "SELECT available_quantity FROM books WHERE id=%s", [form.book_id.data]
        )
        result = cur.fetchone()
        ...
        cur.execute(
            "INSERT INTO transactions (book_id,member_id,per_day_fee) VALUES (%s, %s, %s)",
            [
                form.book_id.data,
                form.member_id.data,
...
def return_book(transaction_id):
    ...
    if request.method == "POST" and form.validate():
        ...
        cur.execute(
            "SELECT outstanding_debt,amount_spent FROM members WHERE id=%s",
